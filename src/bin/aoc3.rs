...
fn set_at_coord(bv: &mut BitVec, x: usize, y: usize, v: bool) {
    bv.set(coord_to_index(x,y), v);
}

fn get_at_coord(bv: &BitVec, x: usize, y: usize) -> bool {
    *bv.get(coord_to_index(x,y)).unwrap()
}

fn follow_path(path: &str, mark_vec: &mut BitVec, opt_cmp_vec: Option<&BitVec>) -> Option<u32> {
    let mut x: i32 = 50000;
    let mut y: i32 = 50000;
    ...
    for cmd in path.split(',') {
        ...
        for _ in 0..steps {
            ...
            assert!(x>0 && y>0); // can be converted to usize
            // would like to make this also optional, but
            // getting actual mutable ref from option
            // does not work
            set_at_coord(mark_vec, x as usize, y as usize, true);
            if let Some(v) = opt_cmp_vec {
                if get_at_coord(v, x as usize, y as usize) {
                    let dist = ((50000-x).abs() + (50000-y).abs()) as u32;
                    ...
                    println!("Intersection at ({},{}) dist {}", x,y,dist);
                }
            }
        }
    }
    lowest_intersection_dist
}

fn pathintersections(path1: &str, path2: &str) -> Option<u32> {
    ...
    follow_path(path2, &mut bv2, Some(&bv1)) // XXX: would be nice if bv2 was not needed
}
...
#[test]
fn t_1() {
    assert_eq!(pathintersections("R8,U5,L5,D3", "U7,R6,D4,L4"), Some(6));

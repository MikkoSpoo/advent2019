...
impl Point {
    fn to_index(&self) -> usize {
        ...
        assert!(self.y < 100000);
        let r = self.x as usize + self.y as usize * 100000;
        assert!(r < 10000000000);
        r
    }
    fn dist_to(&self, p: &Point) -> u32 {
        ((p.x as i64 - self.x as i64).abs() +
         (p.y as i64 - self.y as i64).abs()) as u32
    }
...
fn set_at_point(bv: &mut BitVec, p: &Point, v: bool) {
    bv.set(p.to_index(), v);
}

fn get_at_point(bv: &BitVec, p: &Point) -> bool {
    *bv.get(p.to_index()).unwrap()
}


fn lowest_common_steps(m1 :&HashMap<Point,u32>, m2 :&HashMap<Point,u32>) -> Option<u32>
{
    let mut r : Option<u32> = None;
    for (p, steps1) in m1 {
        let steps = steps1 + m2[p];
        match r {
            Some(val) => {
                if val > steps { r = Some(steps); }
            },
            None => r = Some(steps),
        }
    }
...
fn follow_path(path: &str, mark_vec: &mut BitVec, opt_cmp_vec: Option<&BitVec>) ->
              (Option<u32>, HashMap<Point,u32>) {
    let startpoint = Point{x: 50000, y: 50000};
    let mut p = startpoint; // copy
    let mut lowest_intersection_dist : Option<u32> = None;
    let mut intersections: HashMap::<Point,u32> = HashMap::<Point,u32>::new();
    let mut total_steps = 0;
    for cmd in path.split(',') {
        let dir = &cmd[0..1]; // panics for multibyte utf8 char
        let steps = &cmd[1..];
